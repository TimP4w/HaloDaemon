...
mod tests {
    ...
    fn tkl_ansi_enter_is_in_home_row() {
        ...
        let enter = cells
            .iter()
            .find(|c| c.id == KeyId::Enter)
            .expect("Enter missing");
        assert_eq!(
            enter.row, 4.5,
            "ANSI Enter must be in the home row (row 4.5)"
        );
    }
    ...
    fn tkl_iso_enter_is_in_qwerty_row() {
        ...
        assert_eq!(
            enter.row, 3.0,
            "ISO Enter LED must be in the QWERTY row (upper arm)"
        );
    }
    ...
    fn tkl_iso_backslash_is_in_home_row() {
        ...
        let bs = cells
            .iter()
            .find(|c| c.id == KeyId::Backslash)
            .expect("Backslash missing");
