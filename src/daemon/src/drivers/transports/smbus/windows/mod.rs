...
//! This is a direct port of the Python reference transports
//! (`refs/src/transports/smbus.py` and `refs/src/transports/nvapi_smbus.py`).
//!
//! Runtime requirements:
//!   - PawnIO installed, with `SmbusI801.bin` (Intel) / `SmbusPIIX4.bin` (AMD)
//!     placed next to the executable or in a `pwnio/` subfolder.
...
// ── SmBusInner — chipset (PawnIO) or GPU (NvAPI) ──────────────────────────────
