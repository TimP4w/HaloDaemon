...
// Offset keeps GPU bus numbers disjoint from chipset buses (numbered from 0).
...
/// Absolute path to `nvapi64.dll` under System32, avoiding the Windows DLL
/// search-order hijack risk (CWD / %PATH%).
fn nvapi_dll_path() -> String {
...
fn init_nvapi() -> Option<NvApi> {
    let sys32 = nvapi_dll_path();
    let lib = unsafe { libloading::Library::new(&sys32) }.ok()?;

    type QueryFn = unsafe extern "C" fn(u32) -> usize;
    let query: libloading::Symbol<QueryFn> = unsafe { lib.get(b"nvapi_QueryInterface\0") }.ok()?;
...
impl GpuBus {
    ...
    fn xfer(
        ...
        // No SMBus QUICK command in NvAPI; treat it as a no-op success.
        if size == SMBUS_QUICK {
            return Ok(0);
        }
    ...
    pub(super) fn write_quick(&mut self, _addr: u8) -> Result<bool> {
        // NvAPI has no QUICK probe — returning true would cause spurious device
        // matches on GPU SMBus buses. Return false so callers use read_byte_data.
        Ok(false)
    }
...
mod tests {
    ...
    #[test]
    fn nvapi_dll_path_is_always_under_system32() {
        let path = nvapi_dll_path();
        assert!(
            path.ends_with(r"\System32\nvapi64.dll"),
            "path must resolve under System32, got: {path}"
        );
        // Never a bare filename / relative path — that would re-open the
        // CWD/%PATH% search-order hijack this function exists to avoid.
        assert!(path.contains(r"\System32\"));
    }
}
