...
use halod_hwaccess::proto::{self, Request, Response, BROKER_SERVICE_NAME, PIPE_NAME};
...
fn try_start_broker_service() -> Result<()> {
...
fn ensure_broker_running() {
    match try_start_broker_service() {
...
fn connect_or_spawn() -> Result<File> {
...
pub fn open_bus(info: &BusInfo) -> Result<Box<dyn SmBusSyncOps + Send>> {
    let mut pipe = connect_or_spawn()?;
    proto::write_frame(&mut pipe, &Request::OpenBus { info: info.clone() })
        .map_err(|e| anyhow!("broker send: {e}"))?;
    let bus_id = expect_opened(
        proto::read_frame(&mut pipe).map_err(|e| anyhow!("broker recv: {e}"))?,
        "smbus",
    )?;

    // Cache block-write support up front so the trait's `&self` accessor is free.
    proto::write_frame(&mut pipe, &Request::SupportsBlockWrite { bus: bus_id })
        .map_err(|e| anyhow!("broker send: {e}"))?;
    let supports_block =
        match proto::read_frame(&mut pipe).map_err(|e| anyhow!("broker recv: {e}"))? {
            Response::Bool(b) => b,
            other => {
