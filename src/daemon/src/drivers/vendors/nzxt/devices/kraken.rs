...
/// Protocol reference: liquidctl kraken2.py/kraken3.py (GPL-3.0), https://github.com/liquidctl/liquidctl
///
use anyhow::Result;
...
use crate::{
    drivers::{
        ...
        vendors::generic::devices::common::{
            build_device_id, per_led_frame, stable_serial, WireDeviceBuilder,
        },
        vendors::nzxt::devices::NzxtFanHub,
        CapabilityRef, ChainCapability, Device, FanCapability, FanStateSlot, LcdCapability,
        LcdStateSlot, RgbCapability, RgbStateSlot, SensorCapability, VisibilitySlot,
        CHAIN_LINK_KIND_NZXT_ARGB,
    },
    registry::discovery::{DeviceDescriptor, DiscoveryHandle},
};
use halod_shared::commands::ScreenRotation;
use halod_shared::types::{
    DeviceCapability, DeviceType, LcdDescriptor, LcdMode, LedPosition, PumpStatus, RgbColor,
    RgbDescriptor, RgbState, RgbZone, ScreenShape, Sensor, SensorType, SensorUnit, WireDevice,
    ZoneTopology,
};
use halod_shared::zone_transform::transform_colors;

/// NZXT firmware caps the Kraken's accessory header at 96 LEDs per channel.
pub const MAX_NZXT_KRAKEN_CHAIN_LEDS: u32 = 96;

/// The Kraken's single external accessory header, exposed as logical channel `0` in the chain IPC.
...
const FALLBACK_PROFILE: &KrakenProfile = &KRAKEN_PROFILES[2].1;
...
pub struct NZXTKraken {
    ...
    /// True once the raw streaming-mode handshake has run; reset whenever the
    /// panel leaves raw streaming (Q565 frame, image upload, default screen).
    raw_stream_entered: std::sync::atomic::AtomicBool,
...
impl NZXTKraken {
    pub fn new(
        ...
        let arc = Arc::new_cyclic(|weak| Self {
            ...
            raw_stream_entered: std::sync::atomic::AtomicBool::new(false),
    ...
    async fn apply_state(&self, state: &RgbState) -> Result<()> {
        ...
        match state {
            ...
            RgbState::PerLed { zones } => {
                ...
                let zone_leds = zones.get("ring").unwrap_or(&empty);
                ...
                self.protocol.write_ring_frame(&colors).await?;
                if profile.has_logo {
                    let logo_leds = zones.get("logo").unwrap_or(&empty);
...
impl Device for NZXTKraken {
    ...
    fn capabilities(&self) -> Vec<CapabilityRef<'_>> {
        let profile = kraken_profile(self.pid);
        let visible = self.active_state() == halod_shared::types::VisibilityState::Visible;
        let mut caps = vec![
    ...
    fn write_rate_status(&self) -> Option<halod_shared::types::WriteRateStatus> {
        let hid = self.protocol.base.transport.rate_status();
        let bulk = self
            .protocol
            .bulk_transport
            .lock()
            .ok()
            .and_then(|g| g.as_ref().map(|t| t.rate_status()));
        Some(match bulk {
            Some(bulk) => crate::drivers::WriteRateLimiter::combine_status(hid, bulk),
            None => hid,
        })
    }
}
...
impl crate::drivers::Controller for NZXTKraken {
    async fn discover_children(&self) -> Vec<Arc<dyn Device>> {
        ...
        let accessories = self
            ...
            .await
            .unwrap_or_else(|e| {
                log::warn!("[NZXT Kraken] detect_accessories failed: {e:#}");
                Vec::new()
            });
        ...
        'outer: for accessory in accessories {
            ...
            let Some(child) = crate::registry::discovery::make_device(handle) else {
                log::warn!(
...
impl LcdCapability for NZXTKraken {
    ...
    /// Encodes the frame and pushes it over whichever path the device's
    /// `raw_streaming` toggle selects — compressed Q565 (`0x08`, default) or
    /// uncompressed BGR888 (`0x09`, true 24-bit colour). The raw path needs a
    /// one-time streaming-mode handshake, done lazily on the first raw frame.
    async fn stream_frame(&self, rgba: &[u8], width: u32, height: u32) -> Result<()> {
        use std::sync::atomic::Ordering;

        // Firmware rotates only the built-in display, so pre-rotate streamed frames.
        let rotation = self.lcd.rotation();

        // CPU-bound encoding runs on a blocking thread so it never stalls the
        // async runtime (IPC, other engines).
        if self.lcd.raw_streaming() {
            if !self.raw_stream_entered.load(Ordering::Relaxed) {
                self.protocol
                    .enter_streaming_mode(self.lcd.brightness())
                    .await?;
                self.raw_stream_entered.store(true, Ordering::Relaxed);
            }
            ...
            .await?;
            self.protocol
                .stream_frame_raw(self.vid, self.pid, bgr)
                .await
        } else {
            self.raw_stream_entered.store(false, Ordering::Relaxed);
            let degrees: u32 = match rotation {
            ...
            .await??;
            log::trace!(
                "[LCD] q565 encode: {}µs ({} bytes)",
                t_enc.elapsed().as_micros(),
                payload.len()
            );
            self.protocol
    ...
    async fn set_brightness(&self, brightness: u8) -> Result<()> {
        let degrees: u32 = match self.lcd.rotation() {
            halod_shared::types::ScreenRotation::R90 => 90,
            halod_shared::types::ScreenRotation::R180 => 180,
            halod_shared::types::ScreenRotation::R270 => 270,
            _ => 0,
    ...
    async fn reset_to_default(&self) -> Result<()> {
        ...
        // The device cleared its bucket registry; the next upload must re-derive
        // state, and a later raw stream must re-run the handshake.
        *self.protocol.active_bucket.lock().await = None;
        self.raw_stream_entered
            .store(false, std::sync::atomic::Ordering::Relaxed);
    ...
    fn save_state(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "raw_streaming": self.lcd.raw_streaming(),
            "video_path": self.lcd.video_path(),
        })
    }
    ...
    async fn restore_state(&self, v: &serde_json::Value) {
        ...
        if let Some(raw) = v["raw_streaming"].as_bool() {
            self.lcd.set_raw_streaming(raw);
        }
        if let Some(vp) = v.get("video_path") {
            self.lcd
                .set_video_path(serde_json::from_value(vp.clone()).ok().flatten());
        }
        ...
        if let Some(filename) = active_image {
            ...
            match tokio::fs::read(&path).await {
                ...
                Err(e) => {
                    ...
                    if let Err(e) = self.reset_to_default().await {
                        log::warn!("[Kraken LCD] reset_to_default failed: {e}");
                    }
                }
            }
        } else if let Err(e) = self.reset_to_default().await {
            log::warn!("[Kraken LCD] reset_to_default failed: {e}");
        }
    }
}
