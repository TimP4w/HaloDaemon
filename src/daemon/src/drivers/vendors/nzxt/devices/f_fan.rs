...
use crate::{
    ...
    drivers::{
        ...
        CapabilityRef, Device, FanCapability, FanStateSlot, NzxtFanHub, RgbCapability,
        RgbStateSlot, VisibilitySlot,
    },
};
use halod_protocol::types::{
    DeviceType, RgbColor, RgbDescriptor, RgbState, RgbZone, ZoneTopology,
};
use halod_protocol::zone_transform::transform_colors;

inventory::submit!(DeviceDescriptor {
    matches: |h| matches!(h, DiscoveryHandle::NzxtChain { accessory_id, .. }
        if [0x13u8, 0x14, 0x17, 0x18, 0x1B, 0x1C, 0x1D, 0x1E, 0x1F].contains(accessory_id)),
    make: |h| {
        let DiscoveryHandle::NzxtChain {
            channel_id,
            accessory_id,
            chain_hub,
            fan_hub,
            ...
            anyhow::bail!("descriptor matched non-NzxtChain handle");
        };
        Ok(
...
impl Device for NZXTFFan {
    fn id(&self) -> String { format!("nzxt_f_fan_{}_{}", self.fan_hub.id(), self.channel_id) }
    fn name(&self) -> &str { self.device_info().0 }
    fn vendor(&self) -> &str { "NZXT" }
    fn model(&self) -> &str { self.device_info().0 }
...
mod tests {
    ...
    use super::*;
    use async_trait::async_trait;
    use halod_protocol::types::{DeviceCapability, DeviceType};
    ...
    fn zone_transforms_round_trip_in_slot() {
        ...
        // Zone transforms are global (not per-profile) and are stored directly
        // in `rgb_transform_slot`. Verify that `set_zone_transform` → `transform_for`
        // round-trips correctly without going through `save_state`/`load_state`.
        let f = fan(0x13);
