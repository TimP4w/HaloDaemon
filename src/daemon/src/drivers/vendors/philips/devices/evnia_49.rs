...
mod inner {
    ...
    use crate::{
        drivers::{
            ...
            vendors::philips::protocols::philips_evnia::{
                build_extended_set, build_write, PhilipsEvnia49Protocol,
            },
            ActionCapability, BooleanCapability, CapabilityRef, ChoiceCapability, ChoiceStateCache,
            Device, RangeCapability, RangeStateCache,
        },
    ...
    const SCREEN_SAVER_OPTIONS: &[(u8, &str)] = &[(0x00, "Off"), (0x02, "Slow"), (0x03, "Fast")];

    // Standard MCCS VCP 0xD6 (Power Mode): 0x01 = on, 0x04 = standby. We expose
    // only the two states the panel reaches; index order matches the choice.
    const POWER_MODES: &[(u8, &str)] = &[(0x01, "On"), (0x04, "Standby")];
    ...
    pub struct PhilipsEvnia49 {
        ...
        sharpness: AtomicI32,
        gain_red: AtomicI32,
        gain_green: AtomicI32,
        gain_blue: AtomicI32,
        power_mode: AtomicUsize,
    ...
    impl PhilipsEvnia49 {
        fn build_booleans(&self) -> Vec<Boolean> {
            vec![
                Boolean {
        ...
        pub fn new() -> Self {
            Self {
                ...
                gain_red: AtomicI32::new(100),
                gain_green: AtomicI32::new(100),
                gain_blue: AtomicI32::new(100),
                power_mode: AtomicUsize::new(0),
        ...
        async fn read_device_info(&self) {
            ...
            let probes: &[(&str, [u8; 4], fn(&mut DeviceInfo, String))] = &[
                ...
                ("firmware", [0xE1, 0xE6, 0x06, 0x00], |i, s| {
                    i.firmware = Some(s)
                }),
                ("panel_variant", [0xE1, 0xE6, 0x1D, 0x00], |i, s| {
                    i.panel_variant = Some(s)
                }),
                ("panel_id", [0xE1, 0xE8, 0x00, 0x00], |i, s| {
                    i.panel_id = Some(s)
                }),
                ("serial", [0xEF, 0x13, 0x00, 0x20], |i, s| {
                    i.serial = Some(s)
                }),
            ];
        ...
        async fn refresh_from_monitor(&self) {
            ...
            if let Some(v) = try_read!("brightness", self.protocol.get_standard(0x10)) {
                self.brightness
                    .store((v as i32).clamp(0, 100), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("power_led", self.protocol.get_standard(0xF2)) {
                self.power_led
                    .store((v as i32).clamp(0, 4), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("smart_response", self.protocol.get_standard(0xEB)) {
                self.smart_response
                    .store((v as usize).min(3), Ordering::Relaxed);
            }
            if let Some(v) = try_read!("input_source", self.protocol.get_standard(0x60)) {
                self.input_source
                    .store(input_index_from_raw(v), Ordering::Relaxed);
            }
            if let Some(v) = try_read!("osd_language", self.protocol.get_standard(0xCC)) {
                self.osd_language
                    .store(lookup_index(OSD_LANGUAGES, v), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("osd_transparency", self.protocol.get_extended(0x10)) {
                self.osd_transparency
                    .store((v as usize).min(4), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("usb_c_setting", self.protocol.get_extended(0x12)) {
                self.usb_c_setting
                    .store((v as usize).min(1), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("gamma", self.protocol.get_standard(0x72)) {
                self.gamma
                    .store(lookup_index(GAMMA_VALUES, v), Ordering::Relaxed);
            }
            ...
            if let Some(v) = try_read!("light_enhancement", self.protocol.get_extended(0x3D)) {
                self.light_enhancement
                    .store((v as i32).clamp(0, 3), Ordering::Relaxed);
            }
            ...
            // Standard MCCS Video Gain (color balance) and Power Mode.
            if let Some(v) = try_read!("gain_red", self.protocol.get_standard(0x16)) {
                self.gain_red
                    .store((v as i32).clamp(0, 100), Ordering::Relaxed);
            }
            if let Some(v) = try_read!("gain_green", self.protocol.get_standard(0x18)) {
                self.gain_green
                    .store((v as i32).clamp(0, 100), Ordering::Relaxed);
            }
            if let Some(v) = try_read!("gain_blue", self.protocol.get_standard(0x1A)) {
                self.gain_blue
                    .store((v as i32).clamp(0, 100), Ordering::Relaxed);
            }
            if let Some(v) = try_read!("power_mode", self.protocol.get_standard(0xD6)) {
                self.power_mode
                    .store(lookup_index(POWER_MODES, v), Ordering::Relaxed);
            }
        }
                ...
                .capabilities(vec![
                    DeviceCapability::Range(vec![
                        ...
                        Range {
                            ...
                            max: 100,
                            step: 1,
                            value: self.gain_red.load(Ordering::Relaxed),
                            read_only: false,
                            category: "color".into(),
                            start_label: None,
                            end_label: None,
                        ...
                        Range {
                            key: "gain_green".into(),
                            label: "Green Gain".into(),
                            min: 0,
                            max: 100,
                            step: 1,
                            value: self.gain_green.load(Ordering::Relaxed),
                            read_only: false,
                            category: "color".into(),
                            start_label: None,
                            end_label: None,
                        ...
                        Range {
                            key: "gain_blue".into(),
                            label: "Blue Gain".into(),
                            min: 0,
                            max: 100,
                            step: 1,
                            value: self.gain_blue.load(Ordering::Relaxed),
                            read_only: false,
                            category: "color".into(),
                            start_label: None,
                            end_label: None,
                    ...
                    DeviceCapability::Boolean(self.build_booleans()),
                    DeviceCapability::Choice(vec![
                        ...
                        Choice {
                            key: "power_mode".into(),
                            label: "Power".into(),
                            options: choices_from_table(POWER_MODES),
                            selected: self.power_mode.load(Ordering::Relaxed),
                            category: "setup".into(),
                            display: Default::default(),
                            visible_when: None,
                        },
                    ]),
                    DeviceCapability::Action(vec![Action {
        ...
        async fn set_range(&self, key: &str, value: i32) -> Result<()> {
            ...
            match key {
                ...
                "light_enhancement" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x3D, v))
                        .await?;
                ...
                "color_enhancement" => {
                    let v = value.clamp(0, 3) as u8;
                    self.protocol
                        .write_packet(&build_extended_set(0x3E, v))
                        .await?;
                ...
                "osd_h_position" => {
                    let v = value.clamp(0, 100) as u8;
                    self.protocol
                        .write_packet(&build_extended_set(0x0E, v))
                        .await?;
                ...
                "osd_v_position" => {
                    let v = value.clamp(0, 100) as u8;
                    self.protocol
                        .write_packet(&build_extended_set(0x0F, v))
                        .await?;
                ...
                "gain_red" => {
                    let v = value.clamp(0, 100) as u8;
                    self.protocol.write_packet(&build_write(0x16, v)).await?;
                    self.gain_red.store(v as i32, Ordering::Relaxed);
                    Ok(())
                }
                "gain_green" => {
                    let v = value.clamp(0, 100) as u8;
                    self.protocol.write_packet(&build_write(0x18, v)).await?;
                    self.gain_green.store(v as i32, Ordering::Relaxed);
                    Ok(())
                }
                "gain_blue" => {
                    let v = value.clamp(0, 100) as u8;
                    self.protocol.write_packet(&build_write(0x1A, v)).await?;
                    self.gain_blue.store(v as i32, Ordering::Relaxed);
                    Ok(())
                }
        ...
        async fn set_choice(&self, key: &str, selected: usize) -> Result<()> {
            ...
            match key {
                "crosshair" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x04, v))
                        .await?;
                ...
                "osd_transparency" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x10, v))
                        .await?;
                ...
                "osd_timeout" => {
                    let v = selected.min(4) as u8;
                    self.protocol
                        .write_packet(&build_extended_set(0x11, v))
                        .await?;
                ...
                "usb_c_setting" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x12, v))
                        .await?;
                ...
                "kvm" => {
                    let v = selected.min(2) as u8;
                    self.protocol
                        .write_packet(&build_extended_set(0x15, v))
                        .await?;
                    self.kvm.store(v as usize, Ordering::Relaxed);
                    Ok(())
                }
                "power_mode" => {
                    let idx = selected.min(POWER_MODES.len() - 1);
                    let code = POWER_MODES[idx].0;
                    self.protocol.write_packet(&build_write(0xD6, code)).await?;
                    self.power_mode.store(idx, Ordering::Relaxed);
                    Ok(())
                }
                "smart_image" => {
                ...
                "pixel_orbiting" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x34, code))
                        .await?;
                ...
                "screen_saver" => {
                    ...
                    self.protocol
                        .write_packet(&build_extended_set(0x35, code))
                        .await?;
    ...
    impl BooleanCapability for PhilipsEvnia49 {
        ...
        async fn set_boolean(&self, key: &str, value: bool) -> Result<()> {
            match key {
                "adaptive_sync" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x40, value as u8))
                        .await?;
                ...
                "low_input_lag" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x07, value as u8))
                        .await?;
                ...
                "usb_standby" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x13, value as u8))
                        .await?;
                ...
                "smart_power" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x16, value as u8))
                        .await?;
                ...
                "cec" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x17, value as u8))
                        .await?;
                ...
                "auto_warning" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x43, value as u8))
                        .await?;
                ...
                "srgb" => {
                    ...
                    let v = if value { 0x02 } else { 0x00 };
                    self.protocol
                        .write_packet(&build_extended_set(0x20, v))
                        .await?;
    ...
    impl ActionCapability for PhilipsEvnia49 {
        async fn trigger_action(&self, key: &str) -> Result<()> {
            match key {
                "pixel_refresh" => {
                    self.protocol
                        .write_packet(&build_extended_set(0x36, 0x01))
                        .await?;
                    Ok(())
                }
    ...
    mod tests {
        ...
        fn light_enhancement_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x3d, 0x00, 0x03, 0xc6]
            );
        }
        ...
        fn color_enhancement_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x3e, 0x00, 0x03, 0xc5]
            );
        }
        ...
        fn adaptive_sync_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x40, 0x00, 0x01, 0xb9]
            );
        }
        ...
        fn low_input_lag_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x07, 0x00, 0x01, 0xfe]
            );
        }
        ...
        fn audio_mute_frames_match_captures() {
            assert_eq!(
                build_write(0x8D, 1),
                [0x6e, 0x51, 0x84, 0x03, 0x8d, 0x00, 0x01, 0x34]
            );
            assert_eq!(
                build_write(0x8D, 2),
                [0x6e, 0x51, 0x84, 0x03, 0x8d, 0x00, 0x02, 0x37]
            );
        }

        #[test]
        fn rgb_gain_frames_use_standard_vcps() {
            // Standard MCCS Video Gain: red 0x16, green 0x18, blue 0x1A.
            for (vcp, key) in [(0x16u8, "red"), (0x18, "green"), (0x1A, "blue")] {
                let p = build_write(vcp, 80);
                assert_eq!(&p[..7], &[0x6e, 0x51, 0x84, 0x03, vcp, 0x00, 80], "{key}");
                assert_eq!(p[7], ddcci_xor(&p[..7]), "{key}");
            }
        }

        #[test]
        fn power_mode_frames_match_mccs() {
            // 0xD6: 0x01 = on, 0x04 = standby.
            assert_eq!(POWER_MODES[0].0, 0x01);
            assert_eq!(POWER_MODES[1].0, 0x04);
            let standby = build_write(0xD6, 0x04);
            assert_eq!(&standby[..7], &[0x6e, 0x51, 0x84, 0x03, 0xd6, 0x00, 0x04]);
            assert_eq!(standby[7], ddcci_xor(&standby[..7]));
            assert_eq!(lookup_index(POWER_MODES, 0x04), 1);
        }
        ...
        fn smart_response_frames_match_captures() {
            assert_eq!(
                build_write(0xEB, 0),
                [0x6e, 0x51, 0x84, 0x03, 0xeb, 0x00, 0x00, 0x53]
            );
            assert_eq!(
                build_write(0xEB, 1),
                [0x6e, 0x51, 0x84, 0x03, 0xeb, 0x00, 0x01, 0x52]
            );
            assert_eq!(
                build_write(0xEB, 2),
                [0x6e, 0x51, 0x84, 0x03, 0xeb, 0x00, 0x02, 0x51]
            );
            assert_eq!(
                build_write(0xEB, 3),
                [0x6e, 0x51, 0x84, 0x03, 0xeb, 0x00, 0x03, 0x50]
            );
        }
        ...
        fn osd_position_frames_match_captures() {
            ...
            assert_eq!(
                h,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x0e, 0x00, 0x00, 0xf6]
            );
            let v = build_extended_set(0x0F, 0);
            assert_eq!(
                v,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x0f, 0x00, 0x00, 0xf7]
            );
        }
        ...
        fn osd_transparency_frames_match_captures() {
            ...
            assert_eq!(
                t4,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x10, 0x00, 0x04, 0xec]
            );
            let t0 = build_extended_set(0x10, 0);
            assert_eq!(
                t0,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x10, 0x00, 0x00, 0xe8]
            );
        }
        ...
        fn osd_timeout_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x11, 0x00, 0x02, 0xeb]
            );
        }
        ...
        fn usb_c_setting_frames_match_captures() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x12, 0x00, 0x01, 0xeb]
            );
            // High res 2.0 → 6e 51 86 03 e2 a0 12 00 00 ea
            let p = build_extended_set(0x12, 0);
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x12, 0x00, 0x00, 0xea]
            );
        }
        ...
        fn usb_standby_frames_match_captures() {
            ...
            assert_eq!(
                p_off,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x13, 0x00, 0x00, 0xeb]
            );
            let p_on = build_extended_set(0x13, 1);
            assert_eq!(
                p_on,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x13, 0x00, 0x01, 0xea]
            );
        }
        ...
        fn kvm_usb_c_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x15, 0x00, 0x02, 0xef]
            );
        }
        ...
        fn smart_power_frames_match_captures() {
            ...
            assert_eq!(
                off,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x16, 0x00, 0x00, 0xee]
            );
            let on = build_extended_set(0x16, 1);
            assert_eq!(
                on,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x16, 0x00, 0x01, 0xef]
            );
        }
        ...
        fn cec_frames_match_captures() {
            ...
            assert_eq!(
                off,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x17, 0x00, 0x00, 0xef]
            );
            let on = build_extended_set(0x17, 1);
            assert_eq!(
                on,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x17, 0x00, 0x01, 0xee]
            );
        }
        ...
        fn smart_image_hdr_frames_match_captures() {
            ...
            // VCP 0xDC.
            assert_eq!(
                build_write(0xDC, 0x24),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x24, 0x40]
            );
            assert_eq!(
                build_write(0xDC, 0x30),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x30, 0x54]
            );
            assert_eq!(
                build_write(0xDC, 0x21),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x21, 0x45]
            );
            assert_eq!(
                build_write(0xDC, 0x22),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x22, 0x46]
            );
            assert_eq!(
                build_write(0xDC, 0x23),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x23, 0x47]
            );
            assert_eq!(
                build_write(0xDC, 0x20),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x20, 0x44]
            );
        }
        ...
        fn smart_image_sdr_frames_match_captures() {
            ...
            assert_eq!(
                build_write(0xDC, 0x00),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x00, 0x64]
            );
            assert_eq!(
                build_write(0xDC, 0x04),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x04, 0x60]
            );
            assert_eq!(
                build_write(0xDC, 0x08),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x08, 0x6c]
            );
            assert_eq!(
                build_write(0xDC, 0x11),
                [0x6e, 0x51, 0x84, 0x03, 0xdc, 0x00, 0x11, 0x75]
            );
        }
        ...
        fn sharpness_frames_match_captures() {
            assert_eq!(
                build_write(0x87, 0),
                [0x6e, 0x51, 0x84, 0x03, 0x87, 0x00, 0x00, 0x3f]
            );
            assert_eq!(
                build_write(0x87, 100),
                [0x6e, 0x51, 0x84, 0x03, 0x87, 0x00, 0x64, 0x5b]
            );
        }
        ...
        fn gamma_frames_match_captures() {
            assert_eq!(
                build_write(0x72, 0x50),
                [0x6e, 0x51, 0x84, 0x03, 0x72, 0x00, 0x50, 0x9a]
            );
            assert_eq!(
                build_write(0x72, 0x64),
                [0x6e, 0x51, 0x84, 0x03, 0x72, 0x00, 0x64, 0xae]
            );
            assert_eq!(
                build_write(0x72, 0x78),
                [0x6e, 0x51, 0x84, 0x03, 0x72, 0x00, 0x78, 0xb2]
            );
            assert_eq!(
                build_write(0x72, 0x8C),
                [0x6e, 0x51, 0x84, 0x03, 0x72, 0x00, 0x8c, 0x46]
            );
            assert_eq!(
                build_write(0x72, 0xA0),
                [0x6e, 0x51, 0x84, 0x03, 0x72, 0x00, 0xa0, 0x6a]
            );
        }
        ...
        fn color_temp_frames_match_captures() {
            ...
            assert_eq!(
                build_write(0x14, 0x02),
                [0x6e, 0x51, 0x84, 0x03, 0x14, 0x00, 0x02, 0xae]
            );
            assert_eq!(
                build_write(0x14, 0x04),
                [0x6e, 0x51, 0x84, 0x03, 0x14, 0x00, 0x04, 0xa8]
            );
            assert_eq!(
                build_write(0x14, 0x05),
                [0x6e, 0x51, 0x84, 0x03, 0x14, 0x00, 0x05, 0xa9]
            );
            assert_eq!(
                build_write(0x14, 0x0A),
                [0x6e, 0x51, 0x84, 0x03, 0x14, 0x00, 0x0a, 0xa6]
            );
            assert_eq!(
                build_write(0x14, 0x0D),
                [0x6e, 0x51, 0x84, 0x03, 0x14, 0x00, 0x0d, 0xa1]
            );
        }
        ...
        fn pixel_refresh_frame_matches_capture() {
            ...
            assert_eq!(
                p,
                [0x6e, 0x51, 0x86, 0x03, 0xe2, 0xa0, 0x36, 0x00, 0x01, 0xcf]
            );
        }
        ...
        fn parse_get_reply_rejects_bad_checksum() {
            let mut packet = [
                0x6e, 0x88, 0x02, 0x00, 0x10, 0x00, 0x00, 0x64, 0x00, 0x54, 0x00, 0,
            ];
            packet[10] = 0xFF;
        ...
        fn language_index_decodes_english() {
            assert_eq!(lookup_index(OSD_LANGUAGES, 0x0002), 1); // English = entry 1
            assert_eq!(lookup_index(OSD_LANGUAGES, 0x0001), 0); // Chinese (Traditional)
        }

        #[test]
        fn build_booleans_keys_and_categories() {
            let dev = PhilipsEvnia49::new();
            let bools = dev.build_booleans();
            let by_key: std::collections::HashMap<&str, &Boolean> =
                bools.iter().map(|b| (b.key.as_str(), b)).collect();

            assert_eq!(bools.len(), 10);
            // usb_standby groups with the other USB controls.
            assert_eq!(by_key["usb_standby"].category, "system_usb");
            // smart_power groups with the other setup controls.
            assert_eq!(by_key["smart_power"].category, "setup");
            assert!(by_key["hdr_active"].read_only);
            assert!(!by_key["adaptive_sync"].read_only);
        }
    }
}
