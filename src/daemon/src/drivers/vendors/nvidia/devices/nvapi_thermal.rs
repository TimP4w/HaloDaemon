...
pub struct NvThermalSensor {
    ...
    /// `NV_THERMAL_TARGET` (bit-style values): 1=GPU, 2=Memory, 4=Power Supply,
    /// 8=Board, 9=VCD Board, 10=VCD Inlet, 11=VCD Outlet, 15=ALL (used as input).
    pub target: u32,
}
...
pub fn read_temperatures(handle: usize) -> Result<Vec<ThermalReading>> {
    ...
    for sensor in &settings.sensor[..count] {
        let Some(label) = thermal_target_label(sensor.target) else {
            continue;
        };
...
/// Map an `NV_THERMAL_TARGET` value to a friendly label. The enum uses
/// bit-style values (1, 2, 4, 8, …) with gaps; unknown targets return `None`.
fn thermal_target_label(target: u32) -> Option<&'static str> {
    Some(match target {
        1 => "GPU Core",
...
mod tests {
    ...
    #[test]
    fn thermal_target_labels_use_bit_style_enum() {
        // NV_THERMAL_TARGET is 1/2/4/8/9/10/11 — bit-style with gaps at 3/5/6/7.
        assert_eq!(thermal_target_label(1), Some("GPU Core"));
        assert_eq!(thermal_target_label(2), Some("GPU Memory Junction"));
        assert_eq!(thermal_target_label(4), Some("Power Supply"));
        assert_eq!(thermal_target_label(8), Some("GPU Board"));
        assert_eq!(thermal_target_label(9), Some("Visual Computing Board"));
        assert_eq!(thermal_target_label(10), Some("Visual Computing Inlet"));
        assert_eq!(thermal_target_label(11), Some("Visual Computing Outlet"));
        // Gaps and the ALL input sentinel are not output labels.
        for unused in [0, 3, 5, 6, 7, 12, 15] {
            assert_eq!(thermal_target_label(unused), None);
        }
    }
}
