//! Pure HID++ frame encoder for Logitech PER_KEY_LIGHTING (feature 0x8081).
//!
//! Streaming RGB animations naively send one `setIndividualRGBZones` packet per
//! 4 keys (~23 packets for an 87-key board, ~480 pkt/s at 20 fps), which floods
//! the HID++ bus and starves other devices sharing the controller.
//!
//! [`encode_frame`] cuts that to ~2-5 packets/frame by:
//!  * frame diffing — keys whose colour is unchanged are not re-sent;
//!  * run-length encoding equal-colour spans into `setRangeRGBZones` (0x50);
//!  * `setConsecutiveRGBZones` (0x20) for consecutive-id varying runs;
//!  * optional 5-bit/4-bit delta compression (0x30/0x40), gated off until the
//!    wire format is verified against hardware — see [`ENABLE_DELTA`].
...
/// Delta-compressed packets (`0x30`/`0x40`) are disabled: `logi_breathe.pcapng`
/// contains no delta traffic, so the wire bit-layout is unverified. Enable only
/// after confirming the format against hardware (see the plan's Risks section).
...
// TODO: move this into protocol, not sure why this is here
...
pub fn encode_frame(
    ...
    // A zone re-enumeration changes the LED count; drop a stale cache so the
    // whole frame is treated as changed.
    if cache.last.len() != keys.len() {
        cache.last = vec![None; keys.len()];
    }

    // Diff against the cached frame, then sort by zone id so equal-colour and
    // consecutive-id runs become adjacent.
    ...
    while i < blocks.len() {
        ...
        while j < blocks.len()
            && blocks[j].dirty
            && blocks[j].len == 1
            && (j == i || blocks[j].first_id == blocks[j - 1].first_id + 1)
        {
            j += 1;
        }
        let run: Vec<(u8, RgbColor)> =
            blocks[i..j].iter().map(|b| (b.first_id, b.color)).collect();
    ...
    for chunk in ranges.chunks(RANGES_PER_PACKET) {
        ...
        packets.push(hidpp::build_packet(
            devnum,
            pk_idx,
            FN_SET_RANGE | SWID,
            &params,
...
///
/// Layout (per packet): `[firstId, r0, g0, b0]` then signed per-channel deltas
/// from the previous key, bit-packed MSB-first. NOTE: unverified against
/// hardware — see [`ENABLE_DELTA`].
fn encode_delta(run: &[(u8, RgbColor)], devnum: u8, pk_idx: u8) -> Option<Vec<Vec<u8>>> {
    ...
    // 12 payload bytes after the 4-byte header hold this many delta keys.
    ...
    for group in run.chunks(keys_per_packet) {
        ...
        packets.push(hidpp::build_packet(devnum, pk_idx, func | SWID, &params, true));
    }
    Some(packets)
}
...
mod tests {
    ...
    /// Every packet must be a well-formed 20-byte HID++ long report.
    ...
    fn uniform_frame_is_one_range_plus_frame_end() {
        ...
        // Single range entry covering the whole id span.
    ...
    fn unchanged_frame_emits_nothing() {
        ...
        // Second identical frame: nothing changed.
    ...
    fn consecutive_gradient_uses_set_consecutive() {
        // 10 consecutive ids, every key a distinct colour.
        let keys: Vec<_> = (1u8..=10).map(|id| (id, rgb(id, id * 2, id * 3))).collect();
        let mut cache = PkFrameCache::default();
        let packets = encode_frame(&keys, &mut cache, DEVNUM, PK_IDX);

        // ceil(10/5) consecutive packets + frameEnd.
    ...
    fn scattered_varying_keys_pack_as_range_singletons() {
        // Non-consecutive ids → cannot use setConsecutive; one range entry each.
        ...
        // ceil(5/3) range packets + frameEnd.
        assert_eq!(packets.len(), 3);
        assert_well_formed(&packets);
        assert_eq!(func(&packets[0]), FN_SET_RANGE);
    ...
    fn cache_records_full_frame_not_just_changes() {
        ...
        keys[0].1 = rgb(1, 2, 3);
        encode_frame(&keys, &mut cache, DEVNUM, PK_IDX);
        // Re-encoding the same frame must now diff to nothing.
        assert!(encode_frame(&keys, &mut cache, DEVNUM, PK_IDX).is_empty());
    }
    ...
    fn led_count_change_rebuilds_cache() {
        ...
        // Different LED count → cache discarded → full frame re-sent.
        assert!(!encode_frame(&keys10, &mut cache, DEVNUM, PK_IDX).is_empty());
    }

    #[test]
    fn delta_encodes_smooth_run_into_fewer_packets() {
        // 4-bit deltas (step of 1 per channel) → 9 keys per packet.
        ...
        // setConsecutive would need ceil(18/5) = 4 packets for the same run.
    }

    #[test]
    fn delta_rejects_steep_run() {
        // A 200-unit jump exceeds even the 5-bit range.
        let run = vec![
            (1u8, rgb(0, 0, 0)),
            (2, rgb(200, 0, 0)),
            (3, rgb(0, 0, 0)),
