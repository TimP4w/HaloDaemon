...
//   hhttps://github.com/mijoe/g560-led
...
use crate::{
    ...
    drivers::{
        ...
        vendors::logitech::protocols::hidpp::HidppMessenger,
...
inventory::submit! {
    DeviceDescriptor {
        // Windows splits interface 2 into several HID collections; only the
        // vendor RGB collection (usage page 0xFF43, usage 0x0202) accepts
        // writes. Linux exposes one hidraw node — discovery falls back to it
        // with usage_page=0/usage=0.
        matches: |h| match h {
...
fn build_descriptor() -> RgbDescriptor {
    let zones = ZONES
        ...
        .map(|(i, (id, name, _))| RgbZone {
            ...
            // One LED per zone — centered in the zone's [0,1] LED space.
...
struct SubwooferSlot(std::sync::Mutex<u8>);

impl Default for SubwooferSlot {
    fn default() -> Self {
        SubwooferSlot(std::sync::Mutex::new(50))
    }
}
...
impl LogitechG560 {
    pub fn new(path: &str, serial: Option<&str>, idx: usize) -> Result<Self> {
        let transport = HidTransport::open(path, None, 50, false)?;
    ...
    async fn send_zone_color(&self, zone: u8, c: RgbColor) -> Result<()> {
        self.messenger
            .hidpp_long_fire(0xFF, 0x04, 0x3E, &[zone, 0x01, c.r, c.g, c.b, 0x02])
            .await
    }
...
mod tests {
    ...
    fn test_g560_packet() {
        ...
        let pkt = build_packet(0xFF, 0x04, 0x3E, &[zone, 0x01, r, g, b, 0x02], true);
        assert_eq!(pkt.len(), LONG_LEN);
        assert_eq!(
            &pkt[..10],
            &[0x11, 0xFF, 0x04, 0x3E, 0x02, 0x01, 255, 0, 128, 0x02]
        );
