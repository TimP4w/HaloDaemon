//! Shared helpers for device drivers — stable-ID construction, serial
//! normalisation, the `WireDevice` builder, per-LED frame assembly, and
//! keyboard layout utilities shared across vendors.
...
use halod_protocol::types::{
    ConnectionType, DeviceCapability, DeviceType, EffectParamValue, KeyboardLayout, LedPosition,
    RgbColor, RgbZone, WireDevice, ZoneTopology,
};

use crate::drivers::Device;

/// Build a stable device ID of the form `<prefix>_<serial>`, falling back to
/// `<prefix>_<index>` when the hardware exposes no usable serial. The ID must
/// stay constant across runs for the same physical device, so a real serial is
/// always preferred; the discovery index is only a last resort.
...
fn key_positions(spec: &KeyLayoutSpec, col_max: f32) -> Vec<LedPosition> {
    spec.resolve()
        .iter()
        .filter_map(|cell| {
            let id = spec
...
pub fn tkl_key_positions(spec: &KeyLayoutSpec) -> Vec<LedPosition> {
...
pub fn full_size_key_positions(spec: &KeyLayoutSpec) -> Vec<LedPosition> {
    key_positions(spec, 22.5)
}

/// Swap the `layout` field on a `Keyboard` topology variant, leaving other
/// topology variants unchanged.
pub fn override_keyboard_layout(topology: ZoneTopology, layout: &KeyboardLayout) -> ZoneTopology {
    match topology {
        ZoneTopology::Keyboard { form_factor, .. } => {
            ZoneTopology::Keyboard { form_factor, layout: layout.clone() }
        }
        other => other,
    }
}
...
pub fn effect_color(
    ...
    if let Some(EffectParamValue::Color(c)) = params.get(key) { Some(*c) } else { None }
}

/// Extract a string value from a native-effect param map.
pub fn effect_str<'a>(
    params: &'a HashMap<String, EffectParamValue>,
    key: &str,
) -> Option<&'a str> {
    if let Some(EffectParamValue::Str(s)) = params.get(key) { Some(s.as_str()) } else { None }
}

/// Compute LED positions for a ring or multi-ring topology.
///
/// `count` is the total number of LEDs across all rings. `topology` must be
/// `Ring` or `Rings`; other topologies are not handled and return an empty vec.
...
pub fn linear_rgb_zone(id: &str, name: &str, led_count: usize) -> RgbZone {
    let leds = (0..led_count)
        .map(|i| LedPosition {
            id: i as u32,
            x: if led_count > 1 { i as f32 / (led_count - 1) as f32 } else { 0.5 },
            y: 0.5,
        })
        .collect();
    RgbZone {
...
/// Builder for [`WireDevice`], seeded from a [`Device`]'s identity getters.
///
/// Every driver's `serialize()` repeats the same `id/name/vendor/model`
/// boilerplate; this captures it once. `device_type` defaults to
/// [`DeviceType::Other`] and `connected` to `true` — set them explicitly.
...
mod tests {
    ...
    fn build_device_id_uses_serial_when_present() {
        assert_eq!(build_device_id("nzxt_hub", Some("ABC123"), 0), "nzxt_hub_ABC123");
    }
    ...
    fn per_led_frame_fills_missing_indices_with_black() {
        ...
        map.insert("0".to_string(), RgbColor { r: 10, g: 20, b: 30 });
        ...
        assert_eq!(frame[0], RgbColor { r: 10, g: 20, b: 30 });
    ...
    fn effect_color_extracts_color_variant() {
        ...
        params.insert("color".to_string(), EffectParamValue::Color(RgbColor { r: 1, g: 2, b: 3 }));
        assert_eq!(effect_color(&params, "color"), Some(RgbColor { r: 1, g: 2, b: 3 }));
    ...
    fn effect_color_returns_none_for_wrong_variant() {
        let mut params = HashMap::new();
        params.insert("color".to_string(), EffectParamValue::Str("red".to_string()));
    ...
    fn effect_str_extracts_str_variant() {
        let mut params = HashMap::new();
        params.insert("speed".to_string(), EffectParamValue::Str("fast".to_string()));
    ...
    fn effect_str_returns_none_for_wrong_variant() {
        let mut params = HashMap::new();
        params.insert("speed".to_string(), EffectParamValue::Color(RgbColor { r: 0, g: 0, b: 0 }));
