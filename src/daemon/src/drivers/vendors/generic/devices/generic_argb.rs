...
use crate::drivers::{
    chain::ChainHub, vendors::generic::devices::common::{per_led_frame, ring_led_positions}, CapabilityRef, Device, RgbCapability,
    RgbStateSlot, VisibilitySlot,
};
use anyhow::Result;
use async_trait::async_trait;
use halod_protocol::types::{DeviceType, LedPosition, RgbColor, RgbDescriptor, RgbState, RgbZone, ZoneTopology};
...
fn topology_to_positions(topology: &ZoneTopology, count: u32) -> Vec<LedPosition> {
    ...
    match topology {
        ZoneTopology::Linear => (0..count)
            .map(|i| LedPosition {
                id: i,
                x: if count > 1 { i as f32 / (count - 1) as f32 } else { 0.5 },
                y: 0.5,
            })
            .collect(),
...
/// Shared state for the unified `GenericArgb`. The authoritative display
/// name lives in the parent's `ChainHost` state — `GenericArgb::serialize`
/// reads it through `ChainHub::link_name`. `fallback_name` is only used when
/// the host has lost the slot (extremely transient).
...
impl GenericArgb {
    ...
    async fn apply_state(&self, state: RgbState) -> Result<()> {
        ...
        match &state {
            ...
            RgbState::NativeEffect { .. } | RgbState::Engine => {}
        }
...
mod tests {
    ...
    fn rings_topology_splits_leds_into_rings() {
        ...
        assert!(first_ring_avg_x < 0.4, "first ring should be on the left: {first_ring_avg_x}");
        assert!(last_ring_avg_x > 0.6, "last ring should be on the right: {last_ring_avg_x}");
    }
