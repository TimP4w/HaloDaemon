...
//!
//! Two register spaces: SuperIO config space (0x2E/0x4E), entered with the
//! magic 0x87 0x87 sequence and used to look up the runtime HWM base address;
//! and the bank-addressed runtime HWM space at `hwm_base + 5`/`+ 6`.
...
impl Nct677xVariant {
    ...
    /// cleared after detection — without clearing, every runtime register reads as 0.
    ...
    pub fn fan_count_regs(self) -> &'static [u16] {
        match self {
            Self::Nct6776F | Self::Nct6771F => &[0x656, 0x658, 0x65A, 0x65C, 0x65E],
    ...
    /// Whether the fan tachometer is a true 16-bit count rather than the
    /// modern 13-bit packing.
    pub fn fan_count_is_16bit(self) -> bool {
        matches!(self, Self::Nct6771F)
    }
    ...
    if variant.needs_io_space_lock_disable() {
        match bus.superio_inb(NUVOTON_HARDWARE_MONITOR_IO_SPACE_LOCK) {
            Ok(options) if options & 0x10 != 0 => {
                ...
                if let Err(e) = bus.superio_outb(NUVOTON_HARDWARE_MONITOR_IO_SPACE_LOCK, cleared) {
...
/// to the modern NCT67xxD family, the NCT6776F, and the NCT610XD.
...
/// Convert NCT677x fan-count register bytes to RPM using the true 16-bit
/// counter scheme `count = (high << 8) | low` used by the oldest NCT6771F.
/// `0x0000` (no fan) and `0xFFFF` (saturated/stalled) both decode to 0 RPM.
pub fn rpm_from_count_bytes_16bit(high: u8, low: u8) -> u32 {
...
pub fn keep_io_unlocked(bus: &LpcIoBus, chip: Detected) -> Result<()> {
    ...
    if options & 0x10 != 0 {
        bus.superio_outb(NUVOTON_HARDWARE_MONITOR_IO_SPACE_LOCK, options & !0x10)?;
    }
    exit(bus, chip.probe_port)?;
    Ok(())
}

/// Read the 16-bit fan-count register pair for `channel` and convert to RPM.
pub fn read_rpm(bus: &LpcIoBus, chip: Detected, channel: u8) -> u32 {
    if chip.hwm_base == 0 {
        return 0;
    }
    let Some(&reg) = chip.variant.fan_count_regs().get(channel as usize) else {
        return 0;
    };
    // High byte at `reg`, low byte at `reg + 1`.
    let high = match read_hwm(bus, chip.hwm_base, reg) {
        Ok(v) => v,
        Err(e) => {
    ...
    let low = match read_hwm(bus, chip.hwm_base, reg + 1) {
        Ok(v) => v,
        Err(e) => {
            log::trace!(
                "[NCT677x] read fan-count low (reg=0x{:03X}) failed: {e}",
                reg + 1
            );
            return 0;
        }
    };
    if chip.variant.fan_count_is_16bit() {
        rpm_from_count_bytes_16bit(high, low)
    } else {
        rpm_from_count_bytes(high, low)
    }
}

/// Read back the PWM output register for `channel` as a 0-100% duty.
pub fn read_duty(bus: &LpcIoBus, chip: Detected, channel: u8) -> u8 {
    if chip.hwm_base == 0 {
        return 0;
    }
    let Some(&reg) = chip.variant.fan_pwm_out_regs().get(channel as usize) else {
        return 0;
    };
    let raw = read_hwm(bus, chip.hwm_base, reg).unwrap_or(0) as u32;
    ((raw * 100 + 127) / 255) as u8
}

/// Read the current fan-control mode byte for `channel`, for later restore.
/// Returns `None` for EC-family chips (their mode register is shared across
/// all channels, so per-channel save/restore is meaningless).
pub fn read_ctrl_mode(bus: &LpcIoBus, chip: Detected, channel: u8) -> Option<u8> {
    if chip.hwm_base == 0 || chip.variant.is_ec_family() {
        return None;
    }
    chip.variant
        .fan_ctrl_mode_regs()
        .get(channel as usize)
        .and_then(|&r| read_hwm(bus, chip.hwm_base, r).ok())
}

/// Restore a previously saved fan-control mode byte on `channel`.
pub fn restore_ctrl_mode(bus: &LpcIoBus, chip: Detected, channel: u8, mode: u8) {
    if chip.hwm_base == 0 || chip.variant.is_ec_family() {
        return;
    }
    if let Some(&reg) = chip.variant.fan_ctrl_mode_regs().get(channel as usize) {
        if let Err(e) = write_hwm(bus, chip.hwm_base, reg, mode) {
            log::warn!("[NCT677x] restore_ctrl_mode(ch={channel}) failed: {e}");
        }
    }
}

/// Switch `channel` to manual control and write `duty` (0-100%). Errors for the
/// EC-family (NCT668x): all channels share one mode register (0xA00) selected by
/// bit `1 << channel`, so a plain write would clobber the other channels' bits.
/// Per-channel manual control there needs a read-modify-write of that bit, which
/// this path does not implement (see `is_ec_family`).
pub fn set_duty(bus: &LpcIoBus, chip: Detected, channel: u8, duty: u8) -> Result<()> {
    keep_io_unlocked(bus, chip)?;
    if chip.hwm_base == 0 {
        return Ok(());
    }
    if chip.variant.is_ec_family() {
        log::warn!(
            "[NCT677x] per-channel manual fan control unsupported on EC-family chip {} (shared mode register)",
            chip.variant.name()
        );
        bail!(
            "{} does not support per-channel manual fan control",
            chip.variant.name()
        );
    }
    let ch = channel as usize;
    let mode_regs = chip.variant.fan_ctrl_mode_regs();
    let cmd_regs = chip.variant.fan_pwm_cmd_regs();
    if let (Some(&mode_reg), Some(&cmd_reg)) = (mode_regs.get(ch), cmd_regs.get(ch)) {
        // 0 = manual control
        write_hwm(bus, chip.hwm_base, mode_reg, 0)?;
        write_hwm(
            bus,
            chip.hwm_base,
            cmd_reg,
            ((duty as u16 * 255) / 100).min(255) as u8,
        )?;
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn rpm_from_count_bytes_16bit_basic() {
        // 16-bit count = (high << 8) | low = 900 ↔ rpm = 1500.
        assert_eq!(rpm_from_count_bytes_16bit(0x03, 0x84), 1500);
    }

    #[test]
    fn rpm_from_count_bytes_16bit_sentinels() {
        // 0x0000 (no fan) and 0xFFFF (saturated/stalled) both decode to 0.
        assert_eq!(rpm_from_count_bytes_16bit(0x00, 0x00), 0);
        assert_eq!(rpm_from_count_bytes_16bit(0xFF, 0xFF), 0);
    }

    #[test]
    fn only_nct6771f_uses_16bit_fan_count() {
        // NCT6771F is the genuine 16-bit part; NCT6776F and the modern
        // NCT67xxD family use the 13-bit packing.
        assert!(Nct677xVariant::Nct6771F.fan_count_is_16bit());
        assert!(!Nct677xVariant::Nct6776F.fan_count_is_16bit());
        assert!(!Nct677xVariant::Nct6799D.fan_count_is_16bit());
        assert!(!Nct677xVariant::Nct610Xd.fan_count_is_16bit());
    }

    #[test]
    fn ec_family_ctrl_mode_register_is_shared() {
        // The EC-family hazard the gate guards against: every channel's
        // control-mode register is the same address, so switching one channel
        // to manual would override all of them. Non-EC families must have
        // distinct per-channel registers.
        for v in [
            Nct677xVariant::Nct6683D,
            Nct677xVariant::Nct6686D,
            Nct677xVariant::Nct6687D,
            Nct677xVariant::Nct6687DR,
        ] {
            assert!(v.is_ec_family());
            let regs = v.fan_ctrl_mode_regs();
            assert!(
                regs.windows(2).all(|w| w[0] == w[1]),
                "{} ctrl-mode regs expected to be a single shared register",
                v.name()
            );
        }
        for v in [
            ...
            for i in 0..regs.len() {
                for j in (i + 1)..regs.len() {
                    assert_ne!(
                        regs[i],
                        regs[j],
                        "{} ctrl-mode regs must be distinct",
                        v.name()
                    );
                }
            }
    ...
    // ── PWM duty conversions ──────────────────────────────────────────────

    /// The read_duty formula: (raw * 100 + 127) / 255
    fn read_duty_val(raw: u8) -> u8 {
        ((raw as u32 * 100 + 127) / 255) as u8
    }

    /// The set_duty formula: (duty as u16 * 255) / 100
    fn set_duty_val(duty: u8) -> u8 {
        ((duty as u16 * 255) / 100).min(255) as u8
    }

    #[test]
    fn pwm_read_duty_0_raw_is_0() {
        assert_eq!(read_duty_val(0), 0);
    }

    #[test]
    fn pwm_read_duty_128_raw_is_50_percent() {
        assert_eq!(read_duty_val(128), 50);
    }

    #[test]
    fn pwm_read_duty_255_raw_is_100_percent() {
        assert_eq!(read_duty_val(255), 100);
    }

    #[test]
    fn pwm_set_duty_0_is_0_raw() {
        assert_eq!(set_duty_val(0), 0);
    }

    #[test]
    fn pwm_set_duty_50_is_127_raw() {
        assert_eq!(set_duty_val(50), 127);
    }

    #[test]
    fn pwm_set_duty_100_is_255_raw() {
        assert_eq!(set_duty_val(100), 255);
    }

    // ── needs_io_space_lock_disable ───────────────────────────────────────

    #[test]
    fn needs_io_space_lock_disable_all_679x_variants() {
        for v in [
            Nct677xVariant::Nct6791D,
            Nct677xVariant::Nct6792D,
            Nct677xVariant::Nct6792DA,
            Nct677xVariant::Nct6793D,
            Nct677xVariant::Nct6795D,
            Nct677xVariant::Nct6796D,
            Nct677xVariant::Nct6796DR,
            Nct677xVariant::Nct6797D,
            Nct677xVariant::Nct6798D,
            Nct677xVariant::Nct6799D,
            Nct677xVariant::Nct6701D,
            Nct677xVariant::Nct5585D,
        ] {
            assert!(
                v.needs_io_space_lock_disable(),
                "{} should need io space lock disable",
                v.name()
            );
        }
    }
    ...
    #[test]
    fn needs_io_space_lock_disable_older_variants_do_not() {
        for v in [
            Nct677xVariant::Nct6771F,
            Nct677xVariant::Nct6776F,
            Nct677xVariant::Nct610Xd,
        ] {
            assert!(
                !v.needs_io_space_lock_disable(),
                "{} should not need io space lock disable",
                v.name()
            );
        }
    }
