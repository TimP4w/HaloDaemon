...
pub fn read_board_info() -> BoardInfo {
    ...
    let rows: Vec<HashMap<String, Variant>> = match conn
...
fn known_board_labels(board: &BoardInfo) -> Option<&'static [&'static str]> {
    ...
    match (mfr.as_str(), product.as_str()) {
        ...
        ("asus" | "asustek" | "asustekcomputerinc", p)
            if p.contains("proartx870e") =>
        {
            Some(&[
                "CPU Fan", "CPU OPT", "Chassis Fan 1", "Chassis Fan 2",
                "Chassis Fan 3", "AIO Pump", "Water Pump",
            ])
        }
        ...
        ("asus" | "asustek" | "asustekcomputerinc", p)
            if p.contains("x870e") && (p.contains("hero") || p.contains("apex") || p.contains("extreme")) =>
        {
            Some(&[
                "CPU Fan", "CPU OPT", "Chassis Fan 1", "Chassis Fan 2",
                "Chassis Fan 3", "AIO Pump", "Water Pump",
...
fn heuristic_label(chip: DetectedChip, channel: u8) -> String {
    let n = chip.fan_count();
...
mod tests {
    ...
    fn heuristic_7_channel_layout() {
        let c = fake_chip(7);
        assert_eq!(heuristic_label(c, 0), "CPU Fan");
        assert_eq!(heuristic_label(c, 1), "CPU OPT");
        assert_eq!(heuristic_label(c, 2), "Chassis Fan 1");
        assert_eq!(heuristic_label(c, 4), "Chassis Fan 3");
        assert_eq!(heuristic_label(c, 5), "AIO Pump");
        assert_eq!(heuristic_label(c, 6), "Water Pump");
    }
    ...
    fn heuristic_8_channel_ec() {
        let c = fake_chip(8);
        assert_eq!(heuristic_label(c, 0), "CPU Fan");
        assert_eq!(heuristic_label(c, 1), "Pump");
        assert_eq!(heuristic_label(c, 2), "Chassis Fan 1");
        assert_eq!(heuristic_label(c, 7), "Chassis Fan 6");
    }
