...
use crate::{
    ...
    drivers::{
        vendors::generic::devices::common::{linear_rgb_zone, per_led_frame},
        CapabilityRef, Device, RgbCapability, RgbStateSlot, VisibilitySlot,
    },
};
use halod_protocol::types::{
    EffectParamDescriptor, EffectParamValue, NativeEffect, ParamKind, RgbColor,
...
// Corsair DRAM controllers respond at 0x58–0x5F and 0x18–0x1F on the chipset bus.
...
inventory::submit!(DeviceDescriptor {
    ...
    make: |h| {
        ...
        Ok(
            Arc::new(CorsairDramDevice::new_uninitialized(Arc::clone(bus), addr))
...
impl CorsairDramDevice {
    pub fn new_uninitialized(bus: Arc<SmBusDevice>, addr: u8) -> Self {
        Self {
            protocol: CorsairDramProtocol::new(bus, addr),
            info: OnceLock::new(),
    ...
    async fn apply_state(&self, state: RgbState) -> Result<()> {
        let info = self
            .info
            .get()
            .expect("CorsairDramDevice used before initialize()");
    ...
    async fn apply_native_effect(
        ...
        let speed =
            corsair_speed_from_str(params.get("speed").and_then(str_param).unwrap_or("medium"));
        let direction = corsair_direction_from_str(
            params
                .get("direction")
                .and_then(str_param)
                .unwrap_or("right"),
...
fn str_param(v: &EffectParamValue) -> Option<&str> {
    if let EffectParamValue::Str(s) = v { Some(s.as_str()) } else { None }
}
...
impl Device for CorsairDramDevice {
    ...
    fn debug_info_extra(&self) -> Vec<(String, String)> {
        let mut items = vec![
            ...
            ("address".to_string(), format!("0x{:02x}", self.protocol.addr())),
...
mod tests {
    ...
    #[test]
    fn device_id_format() {
        let id = format!("corsair-dram-bus{}-addr{:02x}", 3u8, 0x5Au8);
        assert_eq!(id, "corsair-dram-bus3-addr5a");
    }
    ...
    fn speed_from_str_values() {
        use crate::drivers::vendors::corsair::protocols::corsair_dram::{corsair_speed_from_str, CorsairDramSpeed};
        assert!(matches!(
