...
impl<T: Transport> AuraUsbProtocol<T> {
    ...
    pub async fn set_channel_direct(&self, channel: u8) -> Result<()> {
    ...
    pub async fn send_effect(
        ...
        let color = params
            .get("color")
            .and_then(|v| {
                if let EffectParamValue::Color(c) = v {
                    Some(*c)
                } else {
                    None
                }
...
mod tests {
    ...
    async fn send_direct_single_led() {
        ...
        assert_eq!(pkt[0], AURA_HDR);
        assert_eq!(pkt[1], CMD_DIRECT);
        assert_eq!(pkt[2], 0x80); // direct_channel=0, apply bit set
    ...
    async fn set_channel_direct_sends_setmode_direct() {
        let p = proto(vec![]);
        p.set_channel_direct(2).await.unwrap();
    ...
    fn parse_config_extracts_argb_count_and_led_counts() {
        ...
        config[CT_CH_BLOCK_OFF + CT_CH_LEDS_OFF] = 120;
        config[CT_CH_BLOCK_OFF + CT_CH_BLOCK_SZ + CT_CH_LEDS_OFF] = 60;
    ...
    #[tokio::test]
    async fn send_effect_maps_ids_to_mode_bytes() {
        let cases = [
            ("off", MODE_OFF),
            ("breathing", MODE_BREATHING),
            ("spectrum_cycle", MODE_SPECTRUM_CYCLE),
        ...
        for (id, expected_mode) in cases {
            let p = proto(vec![]);
            let params = HashMap::new();
            p.send_effect(id, 0x01, &params).await.unwrap();
            let w = p.transport.written.lock().await;
            assert_eq!(w.len(), 1, "no write for effect '{id}'");
            assert_eq!(w[0][4], expected_mode, "wrong mode for '{id}'");
        }
    }

    #[tokio::test]
    async fn send_effect_defaults_to_white_when_no_color_param() {
        let p = proto(vec![]);
        p.send_effect("breathing", 0x01, &HashMap::new())
            .await
            .unwrap();
        let w = p.transport.written.lock().await;
        assert_eq!(w[0][5], 255); // R
        assert_eq!(w[0][6], 255); // G
        assert_eq!(w[0][7], 255); // B
    }

    #[tokio::test]
    async fn send_effect_unknown_id_returns_err() {
        let p = proto(vec![]);
        let err = p
            .send_effect("non_existent_effect", 0x01, &HashMap::new())
            .await
            .unwrap_err();
        assert!(err.to_string().contains("unknown effect"), "got: {err}");
    }
}
