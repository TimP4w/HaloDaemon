...
/// Name the service is registered under in the SCM.
pub const SERVICE_NAME: &str = "HalodDaemon";
...
fn diag(msg: &str) {
    ...
    if let Ok(mut f) = std::fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(dir.join("halod-service.log"))
    {
        use std::io::Write;
...
/// Register `HalodDaemon` as an auto-starting `LocalSystem` service and
/// start it. Idempotent: re-running over an existing service is a no-op.
pub fn install() -> Result<()> {
    ...
    match manager.create_service(&info, ServiceAccess::CHANGE_CONFIG) {
        ...
        {
            println!("service '{SERVICE_NAME}' already installed");
        }
...
fn insert_dacl_ace(sddl: &str, ace: &str) -> Option<String> {
    let sddl = sddl.trim();
    if !sddl.starts_with("D:") || sddl.contains(ace) {
        return None;
    }
    Some(format!("D:{ace}{}", &sddl["D:".len()..]))
}
