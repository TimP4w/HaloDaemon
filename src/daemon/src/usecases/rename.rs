...
use crate::usecases::ensure_record;
...
async fn rename_chain_link(
    ...
    chain
        .rename_chain_link(&channel_id, child_id, &new_name, app.clone())
        .await?;
...
mod tests {
    ...
    async fn caps_length_at_64_chars() {
        ...
        let cfg = app.config.read().await;
        assert_eq!(
            cfg.known_devices.get("dev_a").map(|r| r.name.chars().count()),
            Some(MAX_NAME_LEN)
        );
    }
    ...
    // ── Present device + blank name → reset to device.name() ────────────────
    ...
    impl Device for NameOnlyDevice {
        ...
        fn vendor(&self) -> &str { "Acme" }
        fn model(&self) -> &str { "Stub" }
        async fn initialize(&self) -> Result<bool> { Ok(true) }
        async fn close(&self) {}
    ...
    impl ChainAdapter for StubAdapter {
        fn parent_id(&self) -> String { self.parent_id.clone() }
    ...
    impl Device for ChainParent {
        fn id(&self) -> String { "parent_x".into() }
        fn name(&self) -> &str { "Parent" }
        fn vendor(&self) -> &str { "Acme" }
        fn model(&self) -> &str { "Hub" }
        async fn initialize(&self) -> Result<bool> { Ok(true) }
        async fn close(&self) {}
        fn capabilities(&self) -> Vec<CapabilityRef<'_>> { vec![CapabilityRef::Chain(self)] }
    }

    impl ChainCapability for ChainParent {
        fn chain_host(&self) -> Option<&Arc<ChainHost>> { Some(&self.host) }
    }

    async fn setup_chain_parent_with_one_link(app: &Arc<AppState>) -> String {
        let host = ChainHost::new(
            Arc::new(StubAdapter { parent_id: "parent_x".into() }),
            ChainLinkKind::GenericAuraArgb,
        );
        let parent: Arc<dyn Device> = Arc::new(ChainParent { host: host.clone() });
        app.devices.lock().await.push(parent.clone());
        let child_id = host
            .add_link(
                "ch0",
                ChainLinkSpec {
                    kind: ChainLinkKind::GenericAuraArgb,
                    name: "Original".into(),
    ...
    async fn chain_link_rename_routes_through_chain_host() {
        ...
        let cfg = app.config.read().await;
        assert!(cfg.known_devices.get(&child_id).is_none());
    }
