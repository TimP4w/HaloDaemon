...
fn process_name_for_pid(pid_path: &Path) -> Option<String> {
    ...
    if WINE_BINARIES.contains(&basename.as_str()) {
        // Only include Wine processes if we can resolve a real game exe.
        // Returning None here drops Wine system processes (explorer, rpcss, etc.)
        // which would otherwise match Steam via inherited SteamAppId env vars.
    ...
    let name = match basename.strip_prefix('.').and_then(|s| s.strip_suffix("-wrapped")) {
...
fn wine_exe_name(pid_path: &Path) -> Option<String> {
    ...
    if let Some(arg) = args.iter().find(|a| {
        ...
        // arg is a Windows path with backslash separators; split on '\' to get
        // the real filename, then strip the .exe suffix.
...
fn flatpak_app_id_from_pid(pid_path: &Path) -> Option<String> {
    ...
    for line in cgroup.lines() {
        if let Some(scope) = line.split('/').last() {
...
mod tests {
    ...
    fn process_name_for_pid_unwraps_nix_wrapped_name() {
        ...
        assert_eq!(process_name_for_pid(dir.path()), Some("firefox".to_string()));
    }
