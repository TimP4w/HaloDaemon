...
use crate::infrastructure::audio::{self, AudioHandle};
use crate::infrastructure::media::{self, MediaHandle};
...
pub struct CustomTemplate {
    ...
    plugin_assets: HashMap<(String, String, u32), crate::domain::plugin::WidgetImageInput>,
    plugin_revision: u64,
...
impl CustomTemplate {
    ...
    pub(super) async fn gather_plugin_sprites(
        &mut self,
        ctx: &TemplateCtx,
        app: &crate::application::state::AppState,
    ) {
        let dt = self
        ...
        for widget in widgets {
            ...
            let input = crate::domain::plugin::WidgetRenderInput {
                widget_id,
                width,
                height,
                time: ctx.t as f32,
                ...
                media_art: media_info.and_then(|info| {
                    info.art.map(|art| crate::domain::plugin::WidgetImageInput {
                        width: art.width(),
                        height: art.height(),
                ...
                images: entry
                    ...
                    .filter_map(|param| {
                        ...
                        Some((
                            filename,
                            crate::domain::plugin::WidgetImageInput {
                                width: frame.width(),
                                height: frame.height(),
...
fn widget_audio(frame: &audio::SpectrumFrame) -> crate::domain::plugin::WidgetAudioInput {
    crate::domain::plugin::WidgetAudioInput {
        level: frame.level,
        flux: frame.flux,
        beat: frame.beat,
        seq: frame.seq,
...
mod tests {
    ...
    fn widget_audio_preserves_the_effect_spectrum_frame() {
        let mut bands = [0.0; crate::infrastructure::audio::BANDS];
        bands[3] = 0.75;
