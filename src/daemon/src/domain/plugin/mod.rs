...
pub use engine::effect_worker::{EffectFrameInput, EffectZoneInput, LedCoord, PluginEffectHandle};
pub use engine::widget_worker::{
    PluginWidgetHandle, WidgetAudioInput, WidgetImageInput, WidgetRenderInput,
};
pub use engine::worker::run_pre_scan;
...
impl Registry {
    ...
    pub fn build_widget_handle(
        &self,
        secrets: &dyn crate::infrastructure::secrets::SecretStore,
        data_bus: std::sync::Arc<crate::application::bus::data_bus::DataBus>,
        catalog_id: &str,
        ...
        let data = engine::data_api::DataRuntime::new(
            data_bus,
...
impl Registry {
    ...
    pub fn refresh_recommendations(
        ...
        gpu_smbus: &[crate::infrastructure::drivers::transports::smbus::BusInfo],
    ) {
        let state = self.snapshot();
...
impl Registry {
    ...
    pub fn make_device(
        ...
        let identity = crate::domain::registry::identity::identity_from_handle(&handle);
        let device = self.match_handle(app, &handle)?;
        let origin = device.conflict_origin();
        Some(Arc::new(
            crate::domain::registry::identity::IdentifiedDevice::new(device, identity, origin),
        ))
    }
        ...
        let device = Arc::new_cyclic(|weak| {
            let mut dev = LuaDevice::new(LuaDeviceParts {
                ...
                worker: LuaDeviceWorker::Spawn(Box::new(LuaDeviceSpawnParts {
                    ...
                    granted,
                    config,
                    data: engine::data_api::DataRuntime::new(
                        app.data_bus.clone(),
