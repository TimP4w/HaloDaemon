...
pub const TABLES_V1: &[TableContract] = &[
    ...
    TableContract {
        ...
        fields: "is_preview(); color(); local_time(); audio(); media(); sensor_info(id); environment(); draw_media_art(buffer, x, y, width, height); measure_text(text, size); measure_text_box(text, width, style); ellipsize_text(text, size, max_width); draw_text(buffer, text, x, y, size, color?); draw_text_box(buffer, text, x, y, width, height, style, color?); fill_rect(buffer, x, y, width, height, color?); fill_rounded_rect(buffer, x, y, width, height, radius, color?); draw_line(buffer, x1, y1, x2, y2, color?); draw_circle(buffer, cx, cy, radius, filled, color?); draw_arc(buffer, cx, cy, radius, thickness, start_degrees, sweep_degrees, cap_radius, color?); draw_triangle(buffer, x1, y1, x2, y2, x3, y3, filled, color?); draw_image(buffer, name, x, y, width, height, fit, shape); draw_asset(buffer, name, x, y, width, height, fit)",
    },
];
