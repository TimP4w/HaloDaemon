...
- A **transport** (HID, SMBus, USB control, PawnIO/LpcIO) moves bytes. Discovery runs the scanners in the `SCANNERS` table in [daemon/src/domain/registry/observers/discovery.rs](src/daemon/src/domain/registry/observers/discovery.rs); each builds a `DiscoveryHandle` (VID/PID, SMBus addr, chain accessory id, …) and `discover_handle()` hands it to the runtime plugin registry, which owns peripheral construction. Which transport backend a matched plugin gets is resolved through the `DESCRIPTORS` table in [daemon/src/domain/plugin/engine/backends/mod.rs](src/daemon/src/domain/plugin/engine/backends/mod.rs) — one `PluginTransportDescriptor` per bus, defined next to the transport it wraps. Both tables are hand-written, so the full set is visible where it's consumed; adding a bus or a discovery root means one new file plus one table line.
...
- `devices.yaml` - known devices, chain layouts, zone transforms
