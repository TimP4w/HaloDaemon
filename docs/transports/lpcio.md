...
**Source:** `src/daemon/src/drivers/transports/lpcio.rs`
...
`LpcIoBus::open` locates `PawnIOLib.dll` from:
- DLL search path
- `C:\Program Files\PawnIO\PawnIOLib.dll`
- `%ProgramFiles%\PawnIO\PawnIOLib.dll`

It then loads `LpcIO.bin` from (in order):
- Executable's directory
- `pwnio/` next to the executable
- `pwnio/` under the current working directory
